...
public class OWLWriter {
    ...
    private Map<String,List<String>> entityDerives;
    ...
    public OWLWriter(String expressSchemaName, Map<String, EntityVO> entities, Map<String, TypeVO> types, Map<String, Set<String>> siblings, List<NamedIndividualVO> enumIndividuals,
                    Map<String, PropertyVO> properties,Map<String,List<String>> entityDerives) {
        ...
        this.entityDerives=entityDerives;
    }

    public void outputOWL(String filePath,String schemaName) {
        try {
            BufferedWriter out = new BufferedWriter(new FileWriter(filePath + schemaName+".ttl"));
    ...
    public void outputOWLSupplement(String filePath,String schemaName) {
        try {
            BufferedWriter out = new BufferedWriter(new FileWriter(filePath + schemaName+"_supplement.ttl"));
            out.write("@base <" + Namespace.IFC + "> .\r\n");
            out.write("@prefix : <" + Namespace.IFC + "#> .\r\n");
            out.write("@prefix ifc: <" + Namespace.IFC + "#> .\r\n");
            out.write("@prefix supply: <http://www.tue.nl/ddss/ifcOWL_supplementary#> .\r\n");
            out.write(getOwl_header());


            Iterator<Entry<String, EntityVO>> it = entities.entrySet().iterator();
            writeEntitiesToOWLSupplyment(it, out);
	...
	private void writeEntitiesToOWLSupplyment(Iterator<Entry<String, EntityVO>> it, BufferedWriter out) throws IOException {
		  while (it.hasNext()) {
	            ...
	            // Write classes
	            out.write("ifc:" + evo.getName() + "\r\n");
	            out.write("\tsupply:isIfcEntity \"true\"^^xsd:boolean");
	            if (evo.getSuperclass() == null) {
	                out.write(" ;\r\n");
	                out.write("\tsupply:isTopLevelIfcEntity \"true\"^^xsd:boolean");
	            }
	            
	            if (entityDerives.get(evo.getName())!=null) {
	            	List<AttributeVO> attrs=evo.getDerivedAttributeList();
	            	List<String> derives=entityDerives.get(evo.getName());
	            	for (String derive:derives) {
	            		for (AttributeVO attr:attrs) {	            		
	            		if (attr.getOriginalName().equalsIgnoreCase(derive)){
	            			out.write(" ;\r\n");
	    	                out.write("\tsupply:hasDeriveAttribute ifc:"+ attr.getLowerCaseName());
	            		}
	            		}
	            	}
	            }
	            ...
	            List<AttributeVO> attributeList=evo.getAttributes();
	            if (attributeList!=null && attributeList.size()>0) {
	            	for (int i=0;i<attributeList.size();i++) {
	            		AttributeVO attr=attributeList.get(i);
	            		out.write("ifc:" + attr.getLowerCaseName() + "\r\n");
	            		out.write("\tsupply:index \""+i+"\"^^xsd:integer");
	            		if (attr.isList()||attr.isArray()) {
	            			 out.write(" ;\r\n");
	     	                out.write("\tsupply:isListOrArray \"true\"^^xsd:boolean");
	            		}
	            		if (attr.isSet()) {
	            			 out.write(" ;\r\n");
	     	                out.write("\tsupply:isSet \"true\"^^xsd:boolean");
	            		}
	            		if (attr.isOptional()) {
	            			out.write(" ;\r\n");
	     	                out.write("\tsupply:isOptional \"true\"^^xsd:boolean");
	            		}
	            		out.write(" .\r\n");
	    	            out.write("\r\n");
	            	}
